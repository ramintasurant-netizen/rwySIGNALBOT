...
def vessel_h(cx, cy, w, h, label, props=None):
    s = Shape(cx - w / 2, cy - h / 2, w, h, "Equipment", text=label, size=7.5, bold=True, weight=1.4, fill=F_EQ,
...
def vessel_v(cx, cy, w, h, label, props=None):
    s = Shape(cx - w / 2, cy - h / 2, w, h, "Equipment", text=label, size=7.5, bold=True, weight=1.4, fill=F_EQ,
              props=props, name=label.split("\n")[0])
    e = w * 0.18
...
def valve(cx, cy, tag, act="none", fo="", w=0.36, h=0.2, orient="h", size_txt="", props=None, label_side="r"):
    """Bow-tie body. act: none | M motor | D diaphragm control | C check."""
    top = 0.34 if act in ("M", "D") else 0.0
    ...
    if orient == "h":
        ...
        if act == "M":
            s.poly([(w / 2, h / 2), (w / 2, h / 2 + 0.16)], nofill=True)
            s.rect(w / 2 - 0.11, h / 2 + 0.16, 0.22, 0.18)
            text(cx, cy + h / 2 + 0.15, "M", size=5.5, bold=True)
        elif act == "D":
            s.poly([(w / 2, h / 2), (w / 2, h / 2 + 0.16)], nofill=True)
            s.arc((w / 2 - 0.17, h / 2 + 0.16), (w / 2 + 0.17, h / 2 + 0.16), -0.15)
            s.poly([(w / 2 - 0.17, h / 2 + 0.16), (w / 2 + 0.17, h / 2 + 0.16)], nofill=True)
        elif act == "C":
        ...
        if act == "D":
            s.poly([(ox + h / 2, w / 2), (ox + h / 2 - 0.12, w / 2)], nofill=True)
            s.arc((ox + h / 2 - 0.12, w / 2 - 0.14), (ox + h / 2 - 0.12, w / 2 + 0.14), -0.12)
            s.poly([(ox + h / 2 - 0.12, w / 2 - 0.14), (ox + h / 2 - 0.12, w / 2 + 0.14)], nofill=True)
        ty = cy
...
def bubble(cx, cy, tag, kind="field", r=0.24, props=None):
    """ISA-5.1: field = circle; dcs = circle in square + bar; sis = diamond in square; logic = hexagon."""
    s = Shape(cx - r, cy - r, 2 * r, 2 * r, "Instruments", text=tag, size=5.6, bold=True, weight=0.9, fill=F_WHITE,
...
def tee(x, y):
    s = Shape(x - 0.05, y - 0.05, 0.1, 0.1, "Process Piping", weight=0.5, fill=C_PROC, line=C_PROC, name="tee")
